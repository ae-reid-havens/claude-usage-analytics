...
- **All-time cost was frozen at whatever rates each day was recorded under** - `daily_snapshots.cost` stores dollars, not tokens, and `getTotalStats()` sums that column straight into the all-time figure. Every pricing entry added late therefore left the days before it permanently wrong: Opus 4.5 through 4.8 days written at the legacy $15/$75, Fable 5 days written at the $3/$15 default, Opus 5 days at $15/$75. Stored costs are now recomputed from `model_usage` token counts and written back whenever they drift from the current price of those tokens, so the database self-heals on the next load after any repricing instead of needing a manual pass. On the database this was developed against the correction was $84,299.50 to $44,056.49 across 199 days.
- **Snapshot write-back blanked the session count** - The persist loop wrote `sessions: 0` unconditionally. It previously only touched new days and today, so the damage was limited; with the self-heal above it would have reached every day. The stored value is now carried through.
